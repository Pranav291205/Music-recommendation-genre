...
MEMORY_SAFE_SONG_COUNT = 10000 # FIX: Reduced song count to 10,000 for memory safety
...
def load_data_and_model():
    """
    Loads model and the first 10,000 songs of data using nrows to prevent OOM errors.
    """
    ...
    try:
        ...
        # FIX: Use nrows to load only the first 10000 rows
...
def get_songs():
    ...
    try:
        # We only sample 100 songs from the loaded 10,000 for display speed
        SAMPLE_SIZE = 100
        
        if len(song_data) > SAMPLE_SIZE:
            # We take a simple head/slice, since the first 10,000 were already loaded
...
def recommend():
    ...
    try:
        ...
        if selected_song_row.empty:
            # The song must be in the first 10,000 loaded songs to be found
